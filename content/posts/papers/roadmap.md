...
| 논문 | 역할 |
|------|------|
| [DQN](./DQN-playing-atari-with-deep-reinforcement-learning) | Experience Replay + CNN Q-learning, value-based 강화학습 원형 (DeepMind 2013) |
| [PPO](./proximal-policy-optimization) | Clipped surrogate objective로 안정적 policy gradient, CARLA 기반 AV policy 학습 표준 (OpenAI 2017) |
...
| 논문 | 핵심 아이디어 |
|------|-------------|
| [DDPM](./denoising-diffusion-probabilistic-models) | Markov chain 기반 노이즈 추가/제거로 고품질 이미지 합성, DriveDreamer·MagicDrive의 직접 기반 (NeurIPS 2020) |
...
8. [DQN](./DQN-playing-atari-with-deep-reinforcement-learning) → [PPO](./proximal-policy-optimization) — 강화학습 원류 (DQN: value-based, PPO: policy gradient)
9. [nuPlan](./nuPlan) → [NAVSIM](./NAVSIM) → [Bench2Drive](./bench2drive-multi-ability-benchmarking-e2e-autonomous-driving) — 평가 체계
10. [DDPM](./denoising-diffusion-probabilistic-models) — Diffusion 생성 모델 기반
11. [GAIA-1](./GAIA-1) — 생성형 World Model로의 패러다임 전환
12. [DriveDreamer](./DriveDreamer) — 실제 데이터 기반 합성 데이터 생성
13. [MagicDrive](./magicdrive-street-view-generation-3d-geometry-control) — 3D 기하 조건 기반 멀티카메라 생성
14. [DriveArena](./DriveArena) — 생성형 폐루프 시뮬레이션의 통합
