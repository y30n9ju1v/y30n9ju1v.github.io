...
[데이터/시뮬레이터]
 nuScenes / Waymo / CARLA / nuPlan
         ...
         ▼
[인식·표현: BEV]
 OFT → Lift-Splat-Shoot → BEVDet → BEVDepth → BEVFormer
 DETR3D (top-down query 방식, 별개 계보)
         │
         ▼
[점유 예측]
 Occ3D (2023) — 3D 점유 벤치마크
         │
         ▼
[센서 융합 & E2E]
...
[예측·계획]
 UniAD → VAD
         ...
         ▼
[평가 & 강화학습]
 DQN (2013) / NAVSIM / nuPlan / Bench2Drive
         │
         ▼
[World Model & 생성형 시뮬레이션]
 GAIA-1 (2023) / DriveDreamer (2023)
         │
         ▼
[3D 장면 표현]
 NeRF
  ├─► EmerNeRF (2023) — 자기지도 동적 장면 분해
  └─► Mip-NeRF 360
       └─► Instant-NGP
            └─► 3DGS ──────────────────┐
                 ├─► 4D-GS             │
                 ...
                 └─► OmniRe (2025)     │
                          ...
                          [Neural Simulation]
                           ...
                           NeuRAD (2024) — NeRF 기반 다중 데이터셋 범용 AD 시뮬
                           HUGSIM (2024) — 3DGS 기반 실시간
                           OmniRe (2025) — Holistic 동적 객체 통합
```
...
3DGS 기반 장면 재구성 (4D-GS, Street Gaussians, DrivingGaussian, HUGS)
         +
자율주행 에이전트 (UniAD, VAD, TransFuser)
         +
클로즈드루프 평가 프레임워크 (nuPlan, NAVSIM)
         +
생성형 World Model / Video Diffusion (GAIA-1, DriveDreamer)
         │
         ▼
    UniSim (2023)                        HUGSIM (2024)                    GAIA-1 / DriveDreamer (2023)
    ─ NeRF 기반 멀티센서 시뮬레이터       ─ 3DGS 기반 포토리얼 시뮬레이터   ─ 생성형 신경 시뮬레이터
    ─ 카메라 + LiDAR 동시 합성            ─ 실시간 클로즈드루프 평가         ─ 무한 시나리오 생성
    ─ 장면 편집 (액터 추가/제거)           ─ 70+ 시퀀스 벤치마크             ─ 실제 데이터 기반 제어 가능
```
...
2. [OFT](./OFT-monocular-3d-object-detection) → [Lift-Splat-Shoot](./lift-splat-shoot) → [BEVDet](./bevdet-high-performance-multi-camera-3d-object-detection) → [BEVDepth](./bevdepth) → [BEVFormer](./BEVFormer) — BEV 인식 계보
3. [DETR3D](./detr3d-3d-object-detection-multi-view-images) — top-down query 계보
4. [Occ3D](./occ3d-large-scale-3d-occupancy-prediction-benchmark) — 점유 예측으로 확장
5. [TransFuser](./TransFuser) → [BEVFusion](./bevfusion-multi-task-multi-sensor-fusion) — 센서 융합 E2E
6. [UniAD](./uniad-planning-oriented-autonomous-driving) → [VAD](./VAD) — 통합 계획
7. [nuPlan](./nuPlan) → [NAVSIM](./NAVSIM) → [Bench2Drive](./bench2drive-multi-ability-benchmarking-e2e-autonomous-driving) — 평가 체계
8. [GAIA-1](./GAIA-1) — 생성형 World Model로의 패러다임 전환
9. [DriveDreamer](./DriveDreamer) — 실제 데이터 기반 합성 데이터 생성
...
[DQN](./DQN-playing-atari-with-deep-reinforcement-learning) → [UniAD](./uniad-planning-oriented-autonomous-driving) → [GAIA-1](./GAIA-1) → [DriveDreamer](./DriveDreamer) → [UniSim](./unisim-neural-closed-loop-sensor-simulator) → [HUGSIM](./hugsim-real-time-photorealistic-closed-loop-simulator)
