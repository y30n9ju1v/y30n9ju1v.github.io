...
date: 2026-04-19T18:30:00+09:00
draft: false
...
[인식·표현: BEV]
 ...
 DETR3D (top-down query 방식, 별개 계보)
         ...
         ▼
[점유 예측]
 Occ3D (2023) / TPVFormer (2023) — 3D 점유 벤치마크·예측
         │
         ▼
[센서 융합 & E2E]
...
[World Model & 생성형 시뮬레이션]
 GAIA-1 (2023) / DriveDreamer (2023) / MagicDrive (2024) / DriveArena (2024)
         │
         ▼
[3D 장면 표현]
 NeRF
...
생성형 World Model / Video Diffusion (GAIA-1, DriveDreamer)
  ...
  GAIA-1 / DriveDreamer / MagicDrive (2023–2024)
  ─ 생성형 신경 시뮬레이터
  ─ 무한 시나리오 생성
  ─ 실제 데이터 기반 제어 가능

  DriveArena (2024)
  ─ Traffic Manager + World Dreamer 폐루프 통합
  ─ OSM 기반 교통 시뮬 + Diffusion 이미지 생성
  ─ 에이전트 행동 반응형 실시간 시뮬레이션
```
...
2. [OFT](./OFT-orthographic-feature-transform) → [Lift-Splat-Shoot](./lift-splat-shoot) → [BEVDet](./bevdet-high-performance-multi-camera-3d-object-detection) → [BEVDepth](./bevdepth) → [BEVFormer](./BEVFormer) — BEV 인식 계보
3. [DETR3D](./detr3d-3d-object-detection-multi-view-images) — top-down query 계보
4. [Occ3D](./occ3d-large-scale-3d-occupancy-prediction-benchmark) — 점유 예측 벤치마크
...
10. [MagicDrive](./magicdrive-street-view-generation-3d-geometry-control) — 3D 기하 조건 기반 멀티카메라 생성
11. [DriveArena](./DriveArena) — 생성형 폐루프 시뮬레이션의 통합
