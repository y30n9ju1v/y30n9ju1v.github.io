...
[데이터/시뮬레이터]          [인식·표현]               [예측·계획]
 nuScenes / Waymo  ──────►  BEVFormer / OFT  ──────►  UniAD / VAD
 CARLA             ──────►  TransFuser        ──────►  NAVSIM / nuPlan
                                                              │
[3D 장면 표현]                                                ▼
 NeRF ──► 3DGS ──► 3DGS-RT / 3DGUT ──► DrivingGaussian ──► HUGSIM
                    DIFIX3D+ / HUGS
```
...
```
단안 카메라                     다중 카메라
OFT (2018) ─────────────────► BEVFormer (2022)
(직교 피처 변환)                 (Spatial + Temporal Attention)
```
...
정적 배경과 동적 객체를 분리하여 자율주행 장면을 재구성합니다.
...
자율주행 **인식·계획 스택**과 **3DGS 기반 장면 표현**이 만나  
**포토리얼리스틱 클로즈드루프 시뮬레이터**가 탄생합니다.

```
3DGS 기반 장면 재구성 (DrivingGaussian, HUGS)
         +
...
2022  BEVFormer ──────────────────── 멀티카메라 BEV Transformer
      TransFuser ─────────────────── 카메라+LiDAR 융합 E2E
      nuPlan ──────────────────────── 클로즈드루프 계획 벤치마크
2023  3DGS ───────────────────────── 실시간 3D Gaussian 렌더링
      UniAD ───────────────────────── 계획 지향 통합 AV
      VAD ──────────────────────────── 벡터화 장면 표현
2024  3D Gaussian Ray Tracing ────── 레이 트레이싱 3DGS
...
2. [OFT](./OFT-monocular-3d-object-detection) → [BEVFormer](./BEVFormer) — BEV 인식
...
2. [3DGS](./3d-gaussian-splatting) — 실시간 렌더링 혁신
3. [3D Gaussian Ray Tracing](./3d-gaussian-ray-tracing) + [3DGUT](./3dgut-enabling-distorted-cameras-and-secondary-rays-in-gaussian-splatting) — 렌더링 확장
4. [DrivingGaussian](./driving-gaussian-composite-gaussian-splatting) + [HUGS](./hugs-holistic-urban-3d-scene-understanding) — AV 적용
5. [HUGSIM](./hugsim-real-time-photorealistic-closed-loop-simulator) — 시뮬레이터 통합

### 두 분야의 교차점을 빠르게 파악한다면
[3DGS](./3d-gaussian-splatting) → [DrivingGaussian](./driving-gaussian-composite-gaussian-splatting) → [HUGSIM](./hugsim-real-time-photorealistic-closed-loop-simulator) → [NAVSIM](./NAVSIM)
